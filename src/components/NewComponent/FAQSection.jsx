...
//         <div key={index} className="mb-6 bg-one rounded-3xl shadow-md overflow-hidden border-4 border-[#18243A]">
//           <button
//             className="flex shadow-2xl justify-between items-center w-full px-6 py-4 text-left font-medium rounded-2xl text-2xl bg-[#18243A] text-white"
//             onClick={() => setOpenIndex(openIndex === index ? null : index)}
//           >
//             <div className="flex items-center gap-5">
//               <img
//                 src={faq.icon || "/logosvg/faq-icon.svg"}
//                 alt="icon"
//                 className="w-8 h-8"
//               />
//               <span className="text-base lg:text-xl">{faq.question}</span>
//             </div>
//             {openIndex === index ? (
//               <img src="/images/beauty/hair/open.svg" alt="open" className="w-5 h-5 -mr-3" />
//             ) : (
//               <img src="/images/beauty/hair/close.svg" alt="close" className="w-5 h-5 -mr-3" />
//             )}
//           </button>
//           {openIndex === index && (
//             <div className="bg-one text-[#18243A] lg:pl-20 px-4 py-5 font-semibold text-base lg:text-xl">
//               {faq.answer}
//             </div>
//           )}
//         </div>
...
export default function FAQSection({ heading, faqs }) {
  ...
  // ✅ Prevent error if faqs is undefined or empty
  ...
  return (
    <section className="max-w-4xl mx-auto px-4 py-8">
      ...
      {faqs.map((faq, index) => (
        <div
          key={index}
          className="mb-6 bg-one rounded-3xl shadow-md overflow-hidden border-4 border-[#18243A]"
        >
          <button
            className="flex shadow-2xl justify-between items-center w-full px-6 py-4 text-left font-medium rounded-2xl text-2xl bg-[#18243A] text-white"
            onClick={() => setOpenIndex(openIndex === index ? null : index)}
          >
            ...
            {openIndex === index ? (
              <img
                src="/images/beauty/hair/open.svg"
                alt="open"
                className="w-5 h-5 -mr-3"
              />
            ) : (
              <img
                src="/images/beauty/hair/close.svg"
                alt="close"
                className="w-5 h-5 -mr-3"
              />
            )}
